...
// Obtener noticias de la categoría solicitada
$sql = "SELECT id, titulo, subtitulo, autor, fecha_publicacion, Imagen FROM informacion WHERE categoria = ?";
...
<body>
    ...
    <div class="contenedor">
        <header>
            <h1 data-translate>Noticias de "<?php echo htmlspecialchars($categoria); ?>"</h1>
            <h2 data-translate>Explora las noticias de esta categoría</h2>
        </header>
            ...
            <div class="categories">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<a href="../Noticia/noticia.php?id=' . $row["id"] . '">';
                        echo '<img src="' . $imagenSrc . '" alt="' . htmlspecialchars($row["titulo"]) . '">';
                        echo '<h3 data-translate>' . htmlspecialchars($row["titulo"]) . '</h3>';
                        echo '</a>';
                        echo '<div class="actions">';
                        echo '<a href="../Noticia/noticia.php?id=' . $row["id"] . '" class="btn-ver" data-translate>Ver</a>';
                        echo '</div>';
                        echo '</div>';
                    }
                } else {
                    echo "<p data-translate>No hay noticias para esta categoría.</p>";
                }
                ?>
            </div>
        </main>
